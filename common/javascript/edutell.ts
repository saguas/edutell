/// <reference path="../../client/javascript/static/common/variaveis/declare_vars.ts" />
...
Meteor.methods({
  ...
  addInscricao: function(obj, tipo, pass){//fundir esta função com a anterior por forma a poder alterar tipo se conhecer a password
      //console.log("obj ", obj, " tipo ", tipo);
      
      /*if(tipo !== "prof"){
          Meteor.users.update(this.userId,{$set:{"profile.tipo":tipo}});
          dP.update({id:this.userId},{$set:obj});
          return true;
      }else
          return false;
      */
      ...
      /*
      var arrEscolas = obj.escolas;

      if(pass){
          if(verifyAcesso(pass, arrEscolas,tipo)){
              dP.update({id:this.userId},{$set:obj});
              return true;
          }else
              return false;
...
/*
function verifyAcesso(pass, arrEscolas, tipo){
    return _.any(arrEscolas,function(escola){
            console.log("pass ", pass, " escola ", Escolas.findOne({name:escola}));
            var pw = "";
